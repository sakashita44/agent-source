...
文書を、`writing-principles`、`artifact-principles`、`write-docs` の reference に照らして検証する。判定基準はそれらに定め、この Skill は検証の分担と結果の統合を扱う。それらとこの Skill の記述が食い違う場合は、それらの規定を優先し、差異を報告する。
...
4 つのサブエージェントへ分担する。利用可能な同時実行枠まで並列に起動する。各エージェントへ、対象ファイルの一覧、担当する観点、規定元、出力形式を渡す。

担当外の観点についても、気づいた場合は報告してよい。重複する指摘や、読者の判断を変えない指摘は、統合時に除外する。

### 読者と目的

- 規定元: `writing-principles`、`artifact-principles`
- 観点: 説明対象、主な読者、読者の目的、必要十分性、入口文書の責務、暗黙知への依存、用語の自明性、文脈外識別可能性、文書内と文書間の到達経路、参照条件、規定元と要点の抜粋

### 表現と文体

- 規定元: `writing-principles`、`write-docs` の `references/review-examples.md`
- 観点: 簡潔性、用語の一貫性、メタ言及、強調、演出、効能約束、意味上の主体と文法上の主語、指示の終止形、表記、基準時点へ依存する表現、現行仕様と時系列情報の配置、禁止パターン
- 境界: パターン一致だけで違反とせず、例外と文書種別を規定元に基づいて判定する

### 構造

- 規定元: `write-docs` の `references/structure.md`
- 入力: 対象ファイルと構造検査の出力
...
- 規定元: `write-docs` の `references/content-types.md`
