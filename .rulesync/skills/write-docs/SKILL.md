...
1. 対象ファイルと関連文書を読み、説明対象、主な読者、読者の目的、規定元、既存の構成と用語、文書内および文書間の導線を把握する
2. 新規作成や章立ての再編を伴う改稿では、本文の執筆前に構成案を作成して `references/structure.md` で検証する。段落単位の修正、語句の推敲、追記では構成案の作成を省略して直接編集する
...
- `references/structure.md`: 見出し、リスト、情報の順序、構成案の設計時に参照する
- `references/content-types.md`: README、手順、コード例、Reference、設計・アーキテクチャ文書の作成時に参照する
...
漢字検査は日本語を含むファイルのみを対象とする。`--check-reachability` を指定すると、リンク先への到達性もあわせて確認する。ネットワークを利用できない環境では、検査をスキップした旨を報告する。

日本語の文書では、AI 特有の不自然な書式や語彙に関する検査も実行する。検査設定はこの Skill 内に同梱されているため、対象リポジトリへ新たな依存関係や設定ファイルを追加する必要はない。
...
対象リポジトリに同種の検査が備わっている場合は、そちらを優先して実行する。

日本語の自然さと読みやすさは `natural-japanese` が扱う。文体と表現の規則に競合がある場合は `writing-principles` に従う。

検査ツールの出力結果は判断材料とし、機械的な指摘件数のみで判断しない。構造検査の検出件数や階層の形状は、本文の意味上の妥当性を判定したうえで対処を判断する。

## 完了確認

執筆後は対象ファイルを通読し、変更した導線をたどって次の点を確認すること。完了の報告時には、あわせて `refine-docs` による推敲を提案する。
...
- 同じ規則や定義が一箇所で管理され、他の文書の記述がその場の判断に必要な要点に収まっているか
