...
CHANGELOGの`[Unreleased]`をバージョン見出しへ昇格し、タグを打つ。

Git操作の共通原則は`git-general`、コミットの作成は`commit`に従う。
...
指定されている場合も、そのまま採用せずに次を確かめる。1つでも合わない場合は、指定された版と判定した版の差を示して確認する。

- 既存タグの最新版から連続しているか。番号の飛びや降順になっていないか
- `[Unreleased]`の内容と増分が対応しているか。破壊的変更があるのにpatch、追加だけなのにmajorになっていないか
- 既存タグの命名慣行と形式が揃っているか
