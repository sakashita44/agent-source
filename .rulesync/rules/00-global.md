...
合意済みの計画や手順は、その時点の前提に基づくスナップショットである。作業中に前提が変わったことを検知した場合は、当初の計画を維持せず、妥当性を再評価してからユーザーへ示す。

ユーザーが渡した引数、値、対象の指定が、リポジトリの状態や依頼の意図と矛盾する場合は、そのまま実行せず、矛盾の内容を示して確認する。指定を推測で補正しない。

安全かつ依頼範囲内の読み取り、調査、通常の実装は、計画を示した後に自律的に進める。不可逆な操作、外部への影響、追加権限、重大な設計判断、依頼の前提変更が必要な場合に限り、実行前に確認する。

自律して最後まで進めることを目的にしない。判断が分かれる、根拠が手元にない、前提を選ぶ必要がある場合は、推測で先へ進まず、ユーザーへ確認するか、Web調査と外部資料で根拠を取る。確認と調査の手間は、誤った前提で進んだ作業をやり直す手間より小さい。
...
Windows上の作業では、WSLへツールを直接導入しない。WSLでしか実行できない処理は使い捨てコンテナを経由して実行し、コンテナ定義が必要な場合はそのプロジェクトのリポジトリへ置く。プロジェクトに属さない一時的な用途では、定義を残さずに実行し、終了後にコンテナを破棄する。

## Skillの利用

利用可能なSkillを確認し、依頼に適合するものを使用する。
