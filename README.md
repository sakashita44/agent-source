...
使用する Skill に応じて、ホストには次の実行基盤が必要となる。導入と管理の方法はホスト側で決定する。

- Node.js: `write-docs`、`verify-docs`、`deep-research`、`pr-merge` に付属する `.mjs` を実行する場合
- npm の `npx`: `write-docs` の textlint を実行する場合。指定された版のパッケージが対象プロジェクトにない場合は、npm キャッシュへ取得される
...
エージェントがサンドボックス内でコマンドを実行する配布先では、そのサンドボックスから各実行基盤を起動でき、uv のキャッシュなど各実行基盤の管理領域へ書き込めることも必要となる。
