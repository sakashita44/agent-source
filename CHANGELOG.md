...
- `deep-research` の機械検証で、引用とソース数の偽陽性が PASS する問題（`check-report.mjs`）。本文の脚注参照が Sources の定義へ解決し、定義が URL を含むことを検査する。ソース数は query、fragment、パス末尾のスラッシュを除いた URL で数え、`--min-sources` は非負整数の文字列だけを受け付ける。`--ledger` を指定すると、Sources の URL がすべて調査台帳にあることも検査する。リンクの到達性を確認できない場合は `RESULT: UNVERIFIED` と終了コード 2 を返し、成功と区別する
