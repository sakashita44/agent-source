...
- Skill の実行依存と許容するホスト変更の境界（`README.md`、`00-global.md`）。配布後の実行要件を用途別に示す。宣言された実行方法に従うランナーの依存解決と、プロジェクトやホストの設定変更を区別する。Skill の補助スクリプトは Skill のディレクトリを起点に解決し、プロジェクトにも Skill にも属さない Python の実行には `uv run --no-project` を用いる
