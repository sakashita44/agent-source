...
- サブエージェントの委譲判断と実行境界。独立した小規模な調査も原則として委譲し、委譲先の優先順位と fallback 条件、メインとサブの責務、一時ファイル、権限の扱いを `subagent` に集約する。Codex が `agy` の権限省略オプション付き起動を拒否した場合は通常の権限モードで一度だけ再試行し、別の委譲先へ暗黙に切り替えない
