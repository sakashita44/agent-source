...
- Windows 版 Codex で compact 対策の hook が実行されない問題（`hooks.jsonc`）。Codex 向けの各 hook に、`USERPROFILE` からスクリプトを解決する PowerShell 用の `commandWindows` を併記する。`verify.ps1` で、Codex 向けに `commandWindows` が生成されること、Claude Code 向けに含まれないことを検査する
